...
close_price = abs(float(ast.literal_eval(requests.get(f"https://testnet-master-1.lamden.io/contracts/con_{prefix}_dai/balances?key={new_wallet.verifying_key}").content.decode("UTF-8"))['value']['__fixed__']) - 1)
print_color(f"Vault closed for 100 DAI and an additional {close_price} DAI stability fee", color.CYAN)
