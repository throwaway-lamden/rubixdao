...
class VaultTests(unittest.TestCase):
    ...
    def test_state(self):
        try:
            self.vault.change_any_state(key='testing', new_value='testing', signer='me')
            raise
        ...
        try:
            self.vault.change_state(key='testing2', new_value='testing2', signer='me')
            raise
        except AssertionError as message:
            assert 'owner' in str(message)
        try:
            self.vault.change_state(key='testing2', new_value=5) # int is invalid value
        ...
        self.vault.change_state(key='testing2', new_value='0.42', convert_to_decimal=True)
